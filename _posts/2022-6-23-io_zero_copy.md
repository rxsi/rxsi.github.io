...
为了避免每次读写文件时，都需要对硬盘进行读写操作，因此 linux 使用了`PageCache（页缓存）`机制对文件中的数据进行缓存。
当 IO 的访问优先经过 PageCache 时，称为`Buffered IO`。而可以通过使用`O_DIRECT`参数，使访问不经过 PageCache，则称为`Direct IO`。
linux 系统默认使用的是`Buffered IO`
当我们使用`mmap`时，所映射的内核的内存空间也是位于 PageCache 中
...
3. 在`PageCache`中查找对应页的缓存，1）如果页缓存命中，则直接返回文件内容；2）如果没有对应的页缓存，则会产生一个缺页异常。这时系统会创建新的空的页缓存并从磁盘中读取文件内容，更新页缓存，再跳转到读`PageCache`的流程
4. 读取文件成功并返回
...
2. 如果写请求不命中`PageCache`，而且写入的 offset 或者数据的大小不与 4KB 对齐，则会先进行读IO
...
2. 脏页在缓存的时间超过了阈值
3. 用户进程主动调用`sync`或`fsync`系统调用时，会强制进程回写
    - int sync()：不等待写入结果；
...
为了减轻 CPU 的压力，因此发明了`DMA`技术，即`直接内存访问`技术，在进行 **IO 设备和内存数据传输时**，数据的搬运都交给 DMA 控制器，而 CPU 不再需要参与数据的搬运。
因此，现在的数据传输流程是这样的：
