...
### exec函数族
`exec`函数主要用来替换当前进程，系统把代码段替换成新的程序的代码，废弃原有的数据段和堆栈段，重新从`main`函数开始重新执行，只保留`pid`、`ppid`和`nice值`，而生成新的`代码段`、`数据段`、`堆栈段`。对于系统而言，因为进程的pid没有变化，因此仍是原来的进程，但是程序代码已经完成替换。
#### 函数签名
```cpp
...
```
#### 资源影响
对于已经打开的管道和FIFO，将会继续打开，除非已经设置了`**FD_CLOEXEC**`位，这个标志位的意思是在执行了`exec`族函数之后，该文件描述符自动关闭；对于System V消息队列没有任何影响，而Posix消息队列则会被关闭；对于锁则如果该锁具有进程间共享属性将会继续存在，否则消失；对于共享内存（mmap 共享内存、Posix 共享内存、System V 共享内存）都会去除映射。
### fork函数
