...
如果服务端最终因为达到了最大重发次数而转为`CLOSED`状态，那么此时处于`ESTABLISHED`状态的客户端如果没有开启 keepalive 机制（连接空闲2小时之后，开始探测，每次间隔75秒，总共探测9次）或者没有进行任何数据包的发送，那么客户端就会一直处于当前状态，直到进程关闭。否则则会由于心跳超时或者某个数据包重传达到上限而关闭连接，数据包的重传由以下两个参数控制，这两个参数是用以 rto 超时时间的计算：
```shell
