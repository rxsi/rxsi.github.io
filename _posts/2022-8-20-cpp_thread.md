...
```
## 一个进程最多可以创建多少个线程
已知 linux 系统会为每一个线程分配`8MB`的虚拟内存，而 32 位系统中，系统最大内存是 4GB，用户区被分配了 3GB 空间，因此理论上可以创建的线程总数为`3GB / 8MB ≈ 300`。而在 64 位系统中，系统最大内存为 2<sup>64</sup>，用户区占用了 128TB，所以理论上可以创建的线程数是无限的（1000多万），不过我们安装的物理内存一般也就是几十G，所以首先物理内存方面就不支持创建那么多的线程

但是线程的创建实际还受到几个内核参数的限制：
```shell
