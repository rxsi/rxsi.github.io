...
void tcp_shutdown(struct sock *sk, int how)
{
	 ...
	 */
	if (!(how & SEND_SHUTDOWN)) // 如果不是关闭SEND_SHUTDOWN或者SHUTDOWN_MASK，即没有关闭发送端；这里可以看到RCV_SHUTDOWN不会改变tcp状态。经过测试，当主动调用shutdown(fd, RCV_SHUTDOWN)后，如果此时没有数据了，那么会立即返回0；而如果此时又收到了数据，还是可以成功读取。
		return;
